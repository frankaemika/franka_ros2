...
void BalanceController::read_state_from_hardware(JointTrajectoryPoint & state)
{
  const auto joint_num = joint_names_.size();
...
bool BalanceController::read_state_from_command_interfaces(JointTrajectoryPoint & state)
{
...
CallbackReturn BalanceController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  action_server_ = rclcpp_action::create_server<FollowJTrajAction>(
    ...
    std::bind(&BalanceController::goal_callback, this, _1, _2),
    std::bind(&BalanceController::cancel_callback, this, _1),
    std::bind(&BalanceController::feedback_setup_callback, this, _1));
...
CallbackReturn BalanceController::on_activate(const rclcpp_lifecycle::State &)
{
...
CallbackReturn BalanceController::on_deactivate(const rclcpp_lifecycle::State &)
{
...
CallbackReturn BalanceController::on_cleanup(const rclcpp_lifecycle::State &)
{
  // go home
...
CallbackReturn BalanceController::on_error(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
CallbackReturn BalanceController::on_shutdown(const rclcpp_lifecycle::State &)
{
...
rclcpp_action::GoalResponse BalanceController::goal_callback(
...
rclcpp_action::CancelResponse BalanceController::cancel_callback(
...
void BalanceController::feedback_setup_callback(
...
void BalanceController::fill_partial_goal(
...
/*
void BalanceController::sort_to_local_joint_order(
...
bool BalanceController::validate_trajectory_point_field(
...
bool BalanceController::validate_trajectory_msg(
...
void BalanceController::add_new_trajectory_msg(
  const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> & traj_msg)
{
  traj_msg_external_point_ptr_.writeFromNonRT(traj_msg);
}

void BalanceController::preempt_active_goal()
{
  const auto active_goal = *rt_active_goal_.readFromNonRT();
  if (active_goal)
  {
...
void BalanceController::set_hold_position()
{
...
bool BalanceController::contains_interface_type(
...
void BalanceController::resize_joint_trajectory_point(
...
PLUGINLIB_EXPORT_CLASS(
  balance_controller::BalanceController, controller_interface::ControllerInterface)
